...
from .maya_curve_converter import CoordinatePlaneConfig
...
class CurveImportOptionsWidget(QtWidgets.QWidget):
    ...
    def setup_ui(self):
        ...
        curve_desc_label = QtWidgets.QLabel("Curve handling:")
        curve_desc_label.setWordWrap(True)
        main_layout.addWidget(curve_desc_label)
        
        # Add curve mode options
        self.curve_button_group = QtWidgets.QButtonGroup()
        self.curve_radio_buttons = {}
        
        curve_option_frame = QtWidgets.QFrame()
        curve_option_frame.setStyleSheet("QFrame { background-color: #1e1e1e; margin: 2px; border-radius: 4px; }")
        curve_option_layout = QtWidgets.QHBoxLayout(curve_option_frame)
        set_margin_space(curve_option_layout,4,2)
        
        # Get current curve mode from canvas
        separate_curves = True
        if self.canvas:
            separate_curves = self.canvas.get_separate_curves_mode()
        ...
        separate_radio.setChecked(separate_curves)
        
        if self.canvas:
            separate_radio.toggled.connect(lambda checked: self._on_curve_mode_changed(True) if checked else None)
        
        self.curve_radio_buttons['separate'] = separate_radio
        ...
        combine_radio.setChecked(not separate_curves)
        
        if self.canvas:
            combine_radio.toggled.connect(lambda checked: self._on_curve_mode_changed(False) if checked else None)
        
        self.curve_radio_buttons['combine'] = combine_radio
        curve_option_layout.addWidget(combine_radio)
        
        main_layout.addWidget(curve_option_frame)
        #-------------------------------------------------------------------------------------------------------------------
    ...
    def _on_auto_detect_changed(self, enabled):
        ...
        if self.canvas:
            ...
            if enabled:
                print("Auto-detection enabled - will detect coordinate plane from selected curves")
            else:
    ...
    def _on_curve_mode_changed(self, separate_curves):
        """Handle curve mode change"""
        if self.canvas:
            self.canvas.set_separate_curves_mode(separate_curves)
            self.update_settings_display()
    ...
    def refresh_display(self):
        ...
        for mode_key, radio in self.curve_radio_buttons.items():
            if mode_key == 'separate':
                radio.setChecked(separate_curves)
            else:
                radio.setChecked(not separate_curves)
