...
class AnimPickerWindow(QtWidgets.QWidget):
    ...
    def update_button_data(self, button, deleted=False):
        ...
        if deleted:
            # Handle deletions immediately with proper cleanup
            self.initialize_tab_data(current_tab)
            tab_data = DM.PickerDataManager.get_tab_data(current_tab)
            ...
            if new_count < original_count:
                print(f"Successfully deleted button {button.unique_id} from database")
                
                # Update the database with the modified data
                DM.PickerDataManager.update_tab_data(current_tab, tab_data)
                
                # Force the changes to be saved immediately for deletions
                ...
                print(f"Warning: Button {button.unique_id} not found in database for deletion")
                
        else:
            # Handle regular updates with batching only if not currently processing deletions
            if not hasattr(button, '_being_deleted'):
    ...
    def update_canvas_for_tab(self, tab_name):
        ...
        # Set grid visibility and size - ADD THESE LINES
