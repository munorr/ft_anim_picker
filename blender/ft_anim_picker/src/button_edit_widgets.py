# Optimized Button Edit Widgets for Blender Version
# Based on Maya optimization patterns
...
def create_button_edit_widgets(parent):
    ...
    set_margin_space(zorder_layout, 4, 4)
    ...
    set_margin_space(grid_layout, 4, 4)
...
# Optimized helper functions with queuing and batching
def queue_rename_change(main_window, new_label):
    ...
    if isinstance(main_window, UI.BlenderAnimPickerWindow):
        if not main_window.is_updating_widgets:
            main_window.pending_widget_changes['rename'] = new_label
            main_window.widget_update_timer.start(main_window.widget_update_delay)

def queue_opacity_change(main_window, value):
    """Queue opacity changes for batch processing"""
    if isinstance(main_window, UI.BlenderAnimPickerWindow):
        if not main_window.is_updating_widgets:
            main_window.pending_widget_changes['opacity'] = value
            main_window.widget_update_timer.start(50)  # Shorter delay for opacity

def queue_radius_change(main_window, tl, tr, br, bl):
    """Queue radius changes for batch processing"""
    if isinstance(main_window, UI.BlenderAnimPickerWindow):
        if not main_window.is_updating_widgets:
            main_window.pending_widget_changes['radius'] = (tl, tr, br, bl)
            main_window.widget_update_timer.start(main_window.widget_update_delay)
