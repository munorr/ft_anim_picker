...
class IntegerLineEdit(QtWidgets.QLineEdit):
    ...
    def setRange(self, min_value, max_value):
        """Set the minimum and maximum values for the line edit"""
        self.min_value = float(min_value)
        self.max_value = float(max_value)
        
        # Update the validator with new range
        self.setValidator(QDoubleValidator(self.min_value, self.max_value, self.precision, self))
        
        # Clamp current value to new range
        current_value = self.value()
        clamped_value = max(self.min_value, min(self.max_value, current_value))
